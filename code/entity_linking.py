...
class EntityLinking(object):
	...
	def get_cnn_layer(self, input_conv, n_filters, filter_size, conv_stride = [1,1]):
		conv = tf.layers.conv2d(inputs = input_conv,
								filters = n_filters,
	...
	def create_make_embeddings_layer(self):
		with tf.name_scope('mention_cluster'):
			with tf.variable_scope('mention_cluster'):
				avg_pool = tf.reduce_mean(self.mentions_in_cluster, axis=1, keep_dims=True)
				max_pool = tf.reduce_max(self.mentions_in_cluster, axis=1, keep_dims=True)
				self.pooled_m = tf.concat([avg_pool, max_pool], axis=1, name='pooled_m')
					# batch_size x 2 x embeddings_size x 1
				self.cnn_outs_m = self.get_cnn_layer(input_conv = self.pooled_m,
													n_filters = self.embedding_size,
													filter_size = [2,self.embedding_size])
					# batch_size x 1 x 1 x embeddings_size
				self.cluster_emb_m = tf.reshape(self.cnn_outs_m, [-1, self.embedding_size], name='cluster_emb_m')
					# batch_size x embeddings_size
		
		with tf.name_scope('mention_pairs_cluster'):
			with tf.variable_scope('mention_pairs_cluster'):
				avg_pool = tf.reduce_mean(self.mention_pairs_in_cluster, axis=1, keep_dims=True)
				max_pool = tf.reduce_max(self.mention_pairs_in_cluster, axis=1, keep_dims=True)
				self.pooled_p = tf.concat([avg_pool, max_pool], axis=1, name='pooled_p')
					# batch_size x 2 x embeddings_size x 1
				self.cnn_outs_p = self.get_cnn_layer(input_conv = self.pooled_p,
													n_filters = self.embedding_size,
													filter_size = [2,self.embedding_size])
					# batch_size x embeddings_size x 1 x 1 x 1
				self.cluster_emb_p = tf.reshape(self.cnn_outs_p, shape = [-1, self.embedding_size], name='cluster_emb_p') 
					# batch_size x embeddings_size
